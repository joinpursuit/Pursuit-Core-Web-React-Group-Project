...
const getPostReactions = async (req, res, next) => {
  try {
    ...
    res.status(200).json({
      status: "success",
      message: `select reactions from ${req.params.postId}`,
      payload: reaction
    });
  } catch (error) {
    next(error);
...
const newReaction = async (req, res, next) => {
  try {
    let info = req.body;
    let reaction = await dataBase.one(
      "INSERT INTO reactions (user_id, post_id, reaction) VALUES (${user_id}, ${post_id}, ${reaction}) RETURNING *",
      info
    );
    res.status(200).json({
      status: "success",
